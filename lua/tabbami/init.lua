...
function M.get_context()
	...
	local lines_before =
		vim.api.nvim_buf_get_lines(buf, math.max(0, current_line - context_before), current_line, false)
	local lines_after = vim.api.nvim_buf_get_lines(
		buf,
		current_line,
		math.min(vim.api.nvim_buf_line_count(buf), current_line + context_after),
		false
	)
...
Response (single line only):]],
		context.filetype,
		context.before,
		context.current:sub(1, context.cursor_col),
		"<CURSOR_IS_HERE>",
		context.current:sub(context.cursor_col + 1),
		context.after
	)
...
function M.accept_suggestion()
	if current_suggestion then
		...
		-- vim.api.nvim_put( {"ciao"} , "l", true, true)
		vim.schedule(function()
			vim.api.nvim_put({ suggestion_text }, "c", true, true)
		end)
...
function M.trigger_completion()
	...
	vim.notify("Debug: Sending prompt to provider:\n" .. prompt, vim.log.levels.INFO)

	providers.get_completion(prompt, M.config, function(suggestion, error)
		-- Debug logging
		vim.notify("Debug: Completion callback called", vim.log.levels.INFO)
		vim.notify("Debug: Error = " .. vim.inspect(error), vim.log.levels.INFO)
		vim.notify("Debug: Suggestion = " .. vim.inspect(suggestion), vim.log.levels.INFO)

		if error then
			vim.notify("Tabbami errorre: " .. error, vim.log.levels.ERROR)
			return
		end

		if suggestion and suggestion ~= "" then
			vim.notify("Debug: Showing suggestion: " .. suggestion, vim.log.levels.INFO)
			vim.schedule(function()
				M.show_suggestion(suggestion)
			end)
		else
			vim.notify("Debug: No suggestion to show (empty or nil)", vim.log.levels.WARN)
		end
	end)
end
